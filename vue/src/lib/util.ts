...
class Util {
    ...
    title(title: string) {
        let appname = this.abp.localization.localize(
            "AppName",
            appconst.localization.defaultLocalizationSourceName
        );
        let page = this.abp.localization.localize(
            title,
            appconst.localization.defaultLocalizationSourceName
        );
    ...
    setCurrentPath(vm: Vue, name?: string) {
        ...
        if (name === "home") {
            ...
            let currentPathObj = vm.$store.state.app.routers.filter(
                (item: any) => {
                    if (item.children.length <= 1) {
                        return (
                            item.children[0].name === name || item.name === name
                        );
                    } else {
                        let i = 0;
