<template>
    <div class="container">
        <div class="content">
            ...
            <div class="main">
                <div v-if="!!tenant" class="tenant-title">
                    <a @click="showChangeTenant = true">
                        {{ L("CurrentTenant") }}:{{ tenant.name }}
                    </a>
                </div>
                <div v-if="!tenant" class="tenant-title">
                    <a @click="showChangeTenant = true">
                ...
                <div>
                    <Checkbox v-model="loginModel.rememberMe" size="large">
                        {{ L("RememberMe") }}
                    </Checkbox>
                    <a style="float: right; font-size: 14px; margin-top: 3px">
                        {{ L("ForgetPassword") }}
                    </a>
                </div>
                <div style="margin-top: 15px">
                    <Button type="primary" @click="login" long size="large">
                        {{ L("LogIn") }}
                    </Button>
                </div>
...
export default class Login extends AbpBase {
    ...
    rules = {
        userNameOrEmailAddress: [
            {
                required: true,
                message: this.L("UserNameRequired"),
                trigger: "blur",
            },
        ],
        password: [
            {
                required: true,
                message: this.L("PasswordRequired"),
                trigger: "blur",
            },
        ],
    };
