        ...
        >
            <div class="wf-detail">
                <Card dis-hover>
                    <Row>
                        <Col span="24">
                            ...
                            <p>
                                <Tag color="primary" v-if="entity.status == 0">
                                    {{ L("审核中") }}
                                </Tag>
                                <Tag color="success" v-if="entity.status == 2">
                                    {{ L("已完成") }}
                                </Tag>
                                <Tag color="error" v-if="entity.status == 3">
                                    {{ L("已终止") }}
                                </Tag>
                            </p>
                        </Col>
                    ...
                    <Row>
                        <Col span="24">
                            ...
                            >
                                <Row
                                    :gutter="15"
                                    v-for="(row, rowkey) in entity.inputs"
                                    :key="rowkey"
                                >
                                    <Col
                ...
                <Card dis-hover class="margin-top-10">
                    <Timeline>
                        <TimelineItem
                            v-for="(item, key) in entity.executionRecords"
                            :key="key"
                        >
                            <p class="time">{{ item.stepTitle }}</p>
                            <div class="content">
                                <Timeline v-if="auditEntity">
                                    <TimelineItem
                                        v-for="(audit, key) in auditEntity
                                            .auditRecords[
                                            item.executionPointerId
                                        ]"
                                        :key="key"
                                    >
                                        <Avatar
                                            size="small"
                                            slot="dot"
                                            :src="audit.userHeadPhoto"
                                            v-if="audit.userHeadPhoto"
                                        ></Avatar>
                                        <Avatar
                                            size="small"
                                            slot="dot"
